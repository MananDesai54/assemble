...
export function startBackground(canvas: HTMLCanvasElement): Bg {
  ...
  let amber = '#b87a1f', sage = '#43684e', dim = '#6e7566';
  const readTheme = () => {
    const s = getComputedStyle(document.documentElement);
    amber = s.getPropertyValue('--amber').trim() || amber;
    sage = s.getPropertyValue('--ok').trim() || sage;
  ...
  const RIBBONS = [
    { yFrac: 0.28, speed: 0.00045, wl: 0.9, hue: () => amber },
    { yFrac: 0.55, speed: 0.00032, wl: 1.4, hue: () => sage },
    { yFrac: 0.78, speed: 0.00058, wl: 0.7, hue: () => amber },
  ];
