...
import { BlowDetector } from '@assemble/dsp';
import { mulberry32, synthTap } from './fixtures/synth';

const SR = 44100;

function noise(length: number, amp: number, seed = 9) {
...
function feed(det: BlowDetector, samples: Float32Array, startMs = 0) {
  let fired = 0;
  const chunk = 512;
