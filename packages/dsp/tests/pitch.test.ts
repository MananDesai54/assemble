...
import { detectPitch } from '@assemble/dsp';
import { WhistleController } from '@assemble/dsp';
import { mulberry32 } from './fixtures/synth';

const SR = 44100;

function sine(freq: number, length: number, amp = 0.3) {
  const out = new Float32Array(length);
  for (let i = 0; i < length; i++) out[i] = amp * Math.sin((2 * Math.PI * freq * i) / SR);
  return out;
}

function noise(length: number, amp = 0.1, seed = 5) {
...
describe('WhistleController', () => {
  function feed(ctrl: WhistleController, samples: Float32Array, startMs: number) {
    const events: { dir: 1 | -1 }[] = [];
    const chunk = 512;
