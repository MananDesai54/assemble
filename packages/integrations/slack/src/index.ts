...
const appToken = (ctx: IntegrationContext) =>
  ctx.kv.get('slack_app_token') || process.env.SLACK_APP_TOKEN || '';
...
export const slackIntegration: IntegrationManifest = {
  ...
  connectFields: [
    { key: 'slack_user_token', label: 'User token', placeholder: 'xoxp-… user OAuth token', secret: true, help: 'api.slack.com → your app → OAuth & Permissions → User OAuth Token. Listens to what you can read — no bot invites.' },
    { key: 'slack_app_token', label: 'App token (optional — instant push)', placeholder: 'xapp-… Socket Mode token, blank = polling', secret: true, help: 'Optional but recommended: Basic Information → App-Level Tokens (scope connections:write) + Socket Mode on + Event Subscriptions → user events message.channels/groups/im/mpim. Messages then arrive instantly instead of by polling.' },
  ],

  async status() {
    if (!intake) return { connected: false };
    return { connected: true, detail: intake.mode === 'push' ? 'instant push (socket)' : 'polling for new messages' };
  },
